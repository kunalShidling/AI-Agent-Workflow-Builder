...
export default async function dbEventTriggerHandler(req: any, res: any) {
  try {
    ...
    const structuredPayload = {
      ...
      data: {
        new: payload.event.data.new || null,
        old: payload.event.data.old || null
      }
    };
    ...
    for (const trigger of triggersRes.rows) {
      ...
      if (config.table === tableName && config.operation === operation) {
        ...
        try {
          ...
          // Fire the workflow, injecting the structured payload as the input context
          await triggerWorkflowRun(null, trigger.workflow_id, 'database_event', structuredPayload);
          firedCount++;
        } catch (err: any) {
